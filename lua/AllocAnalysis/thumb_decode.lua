	...
	load_store_mt = {
		__index = {
			...
			get_offset = function(self)
				return getOffset(self)
			end,

			get_field_size = function(self)
				return (self:get_type() / 8)
			end
		}
	},

	decode_load_store = function(address)
		local inst = decodeLSIntruction(address)
		return inst and setmetatable(inst, thumb_decode.load_store_mt)
	end
